...
const Board = ({ answers, isShakeRow, setIsShakeRow, isGameEnd }) => {
  const ref = useRef([]);
  ...
  useEffect(() => {
    const count = answers.reduce(
      (acc, { row, completed }) => acc += completed && row.join("") !== word ? 1 : 0, 0);
  ...
  const messages = [
    ...
    "Круто!!!",
    "Уух!!!",
    `Шкода. Загадане слово: ${isGameEnd ? word.toUpperCase() : "І немає чого підглядати в код ))"}`,
    "Не вистачає літер!",
